extends "res://src/entclasses/entclass_character.gd"
...
func _ready() -> void:
	...
	initialize_character_gas(EnemyHumanAttributeSet.new())
...
func get_damage(damage: int = 0) -> void:
	if damage <= 0:
		return
	audio_hurt.play()
	receive_gas_damage(float(damage))
	emit_signal("combatant_damaged", damage)
...
func body_detected(area: Area3D) -> bool:
...
func find_and_move_to(position_point: Vector3) -> void:
	anim.play("idle")
	agent.target_position = position_point
	look_at(position_point, Vector3.UP)
	var next_point := agent.get_next_path_position()
	move_towards_position(next_point, SPEED)
...
func when_cover(delta: float) -> void:
	match CoverType:
		2: anim.play("idle")
		1: anim.play("Crouch")
	if Nearest_cover.get_parent().PlaceAvailable:
		stop()
		global_position = lerp(global_position, CoverPosition, delta * 3.0)
	else:
		State = TAKE_COVER


func nearest_from_player() -> void:
	...
	if player_node and player_node.is_in_group("Player") and player_node.is_quilt_target(self) and hp <= 50:
		...
		if player_node.try_consume_quilt_on_enemy(self):
			blood.emitting = true
			receive_gas_damage(maxf(get_current_health(), 1.0) * 2.0)
			Main.increase_anima()
	else:
		stun_time = 0.4
...
func _on_agr_timer_timeout() -> void:
	if is_see_player():
		CombatMode = true
		emit_signal("equip_weapon")
		State = CHASE
...
func in_cover(pos: Vector3, type: int) -> void:
	State = COVER
	CoverPosition = pos
	CoverType = type
